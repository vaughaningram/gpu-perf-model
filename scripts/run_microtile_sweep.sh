...
if ! command -v nvidia-smi >/dev/null 2>&1; then
    echo "nvidia-smi is unavailable; run inside a Slurm GPU allocation." >&2
    exit 1
fi
...
for size in "${sizes[@]}"; do
    echo "Running microtile2x2 GEMM ${size}x${size}x${size}..." >&2
    result="$("${benchmark}" "${size}" "${size}" "${size}" \
        "${warmups}" "${measured_iterations}" --microtile --csv)"
